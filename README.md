# 湖北汽车工业学院校园失物招领信息管理系统

这是一个用于《面向对象软件开发课程设计》的本地 Web 原型系统，技术栈为 Python + Flask + SQLite + HTML/CSS/JavaScript。

本项目面向湖北汽车工业学院校园场景，提供失物/招领信息发布、分类检索、基于规则的智能匹配、认领申请、管理员审核、信息归档、模拟加急寻物服务、首页公告栏、今日新增统计和日期查找等功能。
...
5. 本项目的 `app.py` 会在启动时自动初始化基础数据表，并在空数据库时自动导入演示账号和演示物品，不会重复插入演示数据。
